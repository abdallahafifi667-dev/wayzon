...
const eventsPublishedCounter = new prometheus.Counter({
  name: "trip_monitoring_events_published_total",
  help: "Total events published to Kafka",
  labelNames: ["event_type"],
});

// Counter: events consumed
const eventsConsumedCounter = new prometheus.Counter({
  name: "trip_monitoring_events_consumed_total",
  help: "Total events consumed from Kafka",
  labelNames: ["event_type"],
});

// Counter: events failed
const eventsFailedCounter = new prometheus.Counter({
  name: "trip_monitoring_events_failed_total",
...
// Gauge: kafka connection status
const kafkaConnectionGauge = new prometheus.Gauge({
  name: "trip_monitoring_kafka_connected",
  help: "Kafka connection status (1=connected, 0=disconnected)",
});

// Histogram: HTTP request duration
const httpRequestDurationHistogram = new prometheus.Histogram({
  name: "users_payment_http_request_duration_seconds",
...
const httpRequestsTotalCounter = new prometheus.Counter({
  name: "users_payment_http_requests_total",
...
class MetricsCollector {
   ...
   */
  static recordEventConsumed(eventType) {
   ...
   */
  static recordEventFailure(eventType, reason) {
    eventsFailedCounter.inc({ event_type: eventType, reason });
    logger.error("Event processing failed", { eventType, reason });
  }
   ...
   */
  static setKafkaConnected(connected) {
    kafkaConnectionGauge.set(connected ? 1 : 0);
    if (connected) {
