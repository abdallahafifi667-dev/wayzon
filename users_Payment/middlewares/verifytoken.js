...
const verifyTokenUpPhoto = async (req, res, next) => {
  ...
  try {
    ...
    // 🔍 Force string ID for database lookup
    const userIdStr = decoded.id ? String(decoded.id) : null;
    
    console.log(`[AUTH] Attempting verification for ID: ${userIdStr}, Role: ${decoded.role}`);

    const User = getUserModel();
    const UserWallet = getUserWalletModel();
    const UserKYC = getUserKYCModel();

    // 1. Fetch Core User - Search by ID first, then fallback to email
    let user = userIdStr ? await User.findById(userIdStr)
      .select("role username email phone avatar country notificationSettings location languages")
      .lean() : null;

    if (!user && decoded.email) {
      console.log(`[AUTH] User not found by ID (${userIdStr}), searching by email: ${decoded.email}`);
      user = await User.findOne({ "email.address": String(decoded.email).toLowerCase() })
        .select("role username email phone avatar country notificationSettings location languages")
        .lean();
    }

    if (!user) {
      console.warn(`[AUTH] ❌ AUTHENTICATION FAILED: No user found for ID: ${userIdStr} or Email: ${decoded.email}`);
      return res.status(403).json({
    ...
    console.log(`[AUTH] ✅ AUTHENTICATION SUCCESS: User found: ${user.username} (${user.email.address || user.email})`);
